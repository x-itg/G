...
const ErrorLoggingSystem = require('./index');

/**
 * 错误日志系统测试文件
 * 验证所有功能是否正常工作
 */
class ErrorLoggingSystemTest {
    constructor() {
    ...
    async initializeTest() {
        ...
        this.logTestResult('初始化测试', 'pass', '错误日志系统初始化成功');
    ...
    async testBasicLogging() {
        ...
        try {
            const errorId = await this.errorSystem.logError({
                level: 'info',
                message: '测试信息日志',
                source: 'test',
                code: 'TEST_INFO'
    ...
    async testErrorLevels() {
        ...
        for (const level of levels) {
            try {
                const errorId = await this.errorSystem.createTestError(level);
    ...
    async testErrorSources() {
        ...
        for (const testError of sources) {
            try {
                const errorId = await this.errorSystem.logError(testError);
    ...
    async testErrorStatistics() {
        ...
        try {
            const stats = this.errorSystem.getErrorStatistics();
    ...
    async testErrorSearch() {
        ...
        try {
            // 创建一些包含特定关键词的错误
            await this.errorSystem.logError({
                level: 'error',
                message: '数据库连接超时错误',
                source: 'database',
                code: 'DB_TIMEOUT'
            ...
            // 搜索包含"错误"的日志
            const searchResults = this.errorSystem.searchErrors('错误');
    ...
    async testErrorExport() {
        ...
        try {
            ...
            for (const format of formats) {
                try {
                    const exportedData = this.errorSystem.exportErrorLogs({}, format);
    ...
    async testErrorResolution() {
        ...
        try {
            ...
            if (errorId) {
                // 标记为已解决
                const resolved = await this.errorSystem.resolveError(
                    errorId, 
                    'test_user', 
                    '这是一个测试解决'
                );
    ...
    async testErrorAggregation() {
        ...
        try {
            ...
            const aggregation = this.errorSystem.getErrorAggregation();
    ...
    async testDatabaseIntegration() {
        ...
        try {
            const dbManager = require('../database/SimplifiedDatabaseManager');
            ...
            // 测试数据库错误记录
            const testErrorId = await db.logError({
                level: 'info',
                message: '数据库集成测试',
                source: 'database',
                code: 'DB_INTEGRATION_TEST'
            });

            if (testErrorId) {
                // 获取错误日志
                const errorLogs = db.getErrorLogs({ limit: 5 });
                
                this.logTestResult('数据库集成', 
                    errorLogs.length > 0 ? 'pass' : 'fail', 
                    `数据库错误日志: ${errorLogs.length} 条记录`);
            } else {
                this.logTestResult('数据库集成', 'fail', '数据库错误记录失败');
            }

        } catch (error) {
            this.logTestResult('数据库集成', 'fail', error.message);
        }
        console.log('');
    }
    ...
    async testGlobalErrorHandlers() {
        ...
        try {
            ...
            this.logTestResult('全局错误处理器', 
                uncaughtExceptionHandled ? 'pass' : 'fail', 
                uncaughtExceptionHandled ? '全局错误处理器工作正常' : '未捕获到警告事件');

        } catch (error) {
            this.logTestResult('全局错误处理器', 'fail', error.message);
        }
        console.log('');
    }
    ...
    async testSystemDiagnostics() {
        ...
        try {
            const diagnostics = await this.errorSystem.runDiagnostics();
    ...
    displayTestResults() {
        ...
        // 显示系统状态
        console.log('\n📊 系统状态:');
        const systemStatus = this.errorSystem.getSystemStatus();
...
if (require.main === module) {
    const test = new ErrorLoggingSystemTest();
...
module.exports = ErrorLoggingSystemTest;